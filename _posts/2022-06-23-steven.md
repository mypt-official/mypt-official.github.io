...
컴플리션을 통해 에러도 방출하는 함수이기에 throws를 이용해 에러를 방출해주도록 작성하였다. 이 방식으로도 사용 시점에 직관적인 코드를 짤 수 있다. 다만 작성 시점에 세마포어 선언해주고 사용해줘야하는 번거로움이 있다. 그리고 동기 함수이기 때문에 쓰레드에 대한 컨트롤은 함수의 실행이 끝날 때 까지 해당 함수가 보유하게 되어 해당 쓰레드에서 다른 동기 함수를 실행할 수 없게 된다.
